...
#aes is an aesthetic #color is based on class


ggplot(data = mpg) +
  geom_bar(mapping = aes(x = class, color = class))
# here you don't need a y, it's auto-aggregated
...
ggplot(data = diamonds.sample, mapping = aes(y = cut, x = carat, size = price)) +
  geom_point()
